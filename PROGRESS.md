...
14. **Видео-стикеры и эмодзи с прозрачностью (сентябрь 2026)**:
    ...
    - Пайплайн: `frames_in → isnet → frames_out → ProRes4444 alpha.mov → overlay на #171A20 → libx264/libvpx-vp9` (`service-animation-prompts.md:1`).
    ...
    - Инструкция для генерации: `service-animation-prompts.md:1` — базовый промпт `mid-grey #808080` + эксперимент A/B/C (тёмный `#171A20` рекомендован для серого металла), негатив `green spill`.