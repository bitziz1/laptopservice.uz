...
12. **Последние правки по ТЗ заказчика (август 2026)**:
   ...
   - **Сроки/цены → сложность (удалено 09.09.2026)**: из `ServiceItem` и `BuildItem` были удалены поля `timeRange`/`warranty`/`price` и временно добавлены бейджи сложности — позже по требованию заказчика параметр сложности полностью убран из `tina/config.ts`, `src/content.config.ts`, `VisualService`, всех `content/services` и `content/builds` markdown, а также из GraphQL-запросов `services/[slug].astro` и `builds/[slug].astro`.
