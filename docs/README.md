...
- [`content-guide.md`](content-guide.md) — правила контента TinaCMS / `content/**/*.md` (single source, symlink `content/CONTENT_GUIDE.md` → `docs/content-guide.md` для Obsidian vault)
...
- Генератор: `python scripts/generate-prompts/generate_animation_prompts.py --out prompts`
- После генерации картинки/видео перетащите в `prompts/Attachments/<slug>/raw-*.mp4` прямо в Obsidian → запустите `./scripts/service-animation/process_service.sh <slug> prompts/Attachments/<slug>/raw-video.mp4` → артефакты в `public/videos/services/`.
